								...
								return hxItem({
									...
									onChange(val) {
										if (!_.isEqual(rowData.name, val)) {
											vm.cptReqQuery[rowIndex].name = val;
										}
									}
								});
							}
								...
								return hxItem({
									...
									onChange(val) {
										if (!_.isEqual(rowData.value, val)) {
											vm.cptReqQuery[rowIndex].value = val;
										}
									}
								});
							}
								...
								return hxItem({
									...
									onChange(val) {
										if (!_.isEqual(rowData.example, val)) {
											vm.cptReqQuery[rowIndex].example = val;
										}
									}
								});
							}
								...
								return hxItem({
									...
									onChange(val) {
										if (!_.isEqual(rowData.desc, val)) {
											vm.cptReqQuery[rowIndex].desc = val;
										}
									}
								});
							}
						...
						defTable.colActions({
							...
							cellRenderer({ rowIndex }) {
								return hxIcon({
									...
									class: "pointer",
									onClick() {
										vm.cptReqQuery.splice(rowIndex, 1);
									}
								});
							}
						})
